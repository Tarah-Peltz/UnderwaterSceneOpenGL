...
unsigned int LoadTexBMP(const char* file)
{
   ...
   unsigned char* image2;      // Image data
   ...
   if (!image2) Fatal("Cannot allocate %d bytes of memory for image %s\n",size,file);
   for (k=0;k<(dx*dy);k+=1)
   {
      image2[k * 4] = image[k * 3 + 2]; //R
      image2[k * 4 + 1] = image[k * 3 + 1]; //G
      image2[k * 4 + 2] = image[k * 3]; //B
      if ((image2[k * 4] < 60 && image2[k * 4 + 1] < 60 && image2[k * 4 + 2] < 60))
         image2[k * 4 + 3] = 0;
     else
         image2[k * 4 + 3] = 255;
   }
   ...
   //  Copy image
   glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA,dx,dy,0,GL_RGBA,GL_UNSIGNED_BYTE,image2);
